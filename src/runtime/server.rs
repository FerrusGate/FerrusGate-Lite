...
use crate::api::{services, middleware as app_middleware};
        ...
        App::new()
            ...
            .service(
                web::scope("/oauth")
                    ...
                    .route("/userinfo", web::get().to(services::oidc_userinfo)
                        .wrap(app_middleware::JwtAuth::new(ctx.jwt_manager.clone(), ctx.cache.clone())))
            )

            // OIDC Discovery 端点（无需认证）
            .service(
                web::scope("/.well-known")
                    .route("/openid-configuration", web::get().to(services::oidc_discovery))
            ...
            .service(
                web::scope("/api/user")
                    ...
                    .route("/authorizations", web::get().to(services::user_list_authorizations))
                    .route("/authorizations/{client_id}", web::delete().to(services::user_revoke_authorization))
            )
    })
    .bind(&bind_addr)?
    .run()
